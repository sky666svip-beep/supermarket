...
const tabData = ref<Record<string, any>>({
  latest: { posts: [], loading: false, finished: false, page: 1, refreshing: false, scrollY: 0 },
  hot: { posts: [], loading: false, finished: false, page: 1, refreshing: false, scrollY: 0 },
  elite: { posts: [], loading: false, finished: false, page: 1, refreshing: false, scrollY: 0 },
})

const onLoad = async (tab: string) => {
  const data = tabData.value[tab]
  try {
    const res = await getPosts({ tab, page: data.page, limit: 10 })
    if (res.success) {
...
const onTabChange = (newTab: string) => {
  ...
  // 保存当前 tab 滚动位置
  tabData.value[activeTab.value].scrollY = window.scrollY
  
  activeTab.value = newTab
  
  // 恢复新 tab 滚动位置
  requestAnimationFrame(() => {
    window.scrollTo(0, tabData.value[newTab].scrollY || 0)
  })
}
...
<template>
  <div class="bg-surface text-on-surface min-h-screen pb-20 antialiased font-body-md">
    ...
    <main class="pt-[104px] px-4 py-4 space-y-4 max-w-3xl mx-auto">
      <div v-for="tab in ['latest', 'hot', 'elite']" :key="tab" v-show="activeTab === tab">
        <van-pull-refresh v-model="tabData[tab].refreshing" @refresh="onRefresh(tab)">
          <van-list
            v-model:loading="tabData[tab].loading"
            :finished="tabData[tab].finished"
            finished-text="没有更多了"
            @load="onLoad(tab)"
          >
            <article 
              v-for="post in tabData[tab].posts" :key="post.id"
            ...
            >
              ...
              <div class="grid grid-cols-3 gap-2 mb-3" v-if="post.parsedImages && post.parsedImages.length > 0">
                <div v-for="(img, idx) in post.parsedImages.slice(0,3)" :key="idx" class="w-full h-24 rounded-lg overflow-hidden border border-surface-variant/30">
                  <img class="w-full h-full object-cover" :src="getThumbnailUrl(img)" />
                </div>
              </div>
              ...
              <div class="flex items-center justify-between text-on-surface-variant font-body-md text-[13px] pt-3 border-t border-surface-variant/30">
                <div class="flex items-center space-x-1 hover:text-primary transition-colors">
                  <i-material-symbols-visibility-outline  class="text-[18px]"></i-material-symbols-visibility-outline>
                ...
                <div class="flex items-center space-x-1 hover:text-primary transition-colors">
                  <i-material-symbols-chat-outline  class="text-[18px]"></i-material-symbols-chat-outline>
    ...
    >
      <i-material-symbols-edit-square-outline  class="font-bold text-[28px]"></i-material-symbols-edit-square-outline>
    </button>
  </div>
</template>
