...
notice.get('/', async (c) => {
    ...
    const conditions = [
        ...
        or(sql `${notices.expiresAt} IS NULL`, sql `${notices.expiresAt} > ${now.getTime()}` // Note: SQLite stores timestamps as ms or s depending on drizzle config. Drizzle timestamp mode uses JS Date which maps to integer ms usually.
...
notice.put('/:id', requireAdmin, async (c) => {
    ...
    if (images !== undefined)
        updates.images = images === null ? null : JSON.stringify(images);
    if (isUrgent !== undefined)
        updates.isUrgent = isUrgent;
    if (expiresAt !== undefined)
        updates.expiresAt = expiresAt === null ? null : new Date(expiresAt);
