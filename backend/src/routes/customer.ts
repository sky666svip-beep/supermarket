...
customer.post('/feedback', requireUser, async (c) => {
  const userId = Number(c.req.header('X-User-Id'))
  const { facilityType, message, images } = await c.req.json()
  const imagesJson = images && images.length > 0 ? JSON.stringify(images) : null
  const result = await db.insert(feedbacks).values({ facilityType, message, images: imagesJson, userId }).returning()
  return c.json(result[0])
})
