...
export const trafficRouter = new Hono<AuthContext>()
...
trafficRouter.get('/:storeId', async (c) => {
  ...
  try {
    ...
    const trafficData = await db.select().from(storeTraffic)
      .where(and(
        eq(storeTraffic.storeId, storeId),
        gte(storeTraffic.createdAt, oneHourAgo)
      ))

    // Aggregate by floor using majority rule
    // Format: { floor: { level1: count, level2: count... } }
    const floorCounts: Record<number, Record<number, number>> = {}
...
trafficRouter.post('/', authMiddleware, async (c) => {
  ...
  try {
    ...
    const existing = await db.select().from(storeTraffic)
      .where(and(
        eq(storeTraffic.storeId, storeId),
        eq(storeTraffic.floor, floor),
        eq(storeTraffic.userId, user.id),
        gte(storeTraffic.createdAt, oneHourAgo)
      ))
      .limit(1)
