...
activityRouter.get('/', async (c) => {
    ...
    try {
        const allActivities = await db.select().from(activities)
            .where(and(eq(activities.isActive, true), or(
        // No start/end time
        and(eq(activities.startTime, null), eq(activities.endTime, null)), 
        // Only start time
        and(lte(activities.startTime, now), eq(activities.endTime, null)), 
        // Only end time
        and(eq(activities.startTime, null), gte(activities.endTime, now)), 
