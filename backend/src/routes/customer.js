...
customer.get('/stores', async (c) => {
    ...
    if (district)
        conditions.push(eq(stores.district, district));
...
customer.get('/regions', async (c) => {
    ...
    allStores.forEach(s => {
        if (!tree[s.province])
            tree[s.province] = {};
        if (!tree[s.province][s.city])
            tree[s.province][s.city] = new Set();
        ...
        return {
            ...
            children: Object.keys(tree[prov]).sort((a, b) => a.localeCompare(b, 'zh-CN')).map(city => {
                return {
                    text: city,
                    value: city,
                    children: Array.from(tree[prov][city]).sort((a, b) => a.localeCompare(b, 'zh-CN')).map(dist => ({
                        text: dist,
                        value: dist
                    }))
                };
